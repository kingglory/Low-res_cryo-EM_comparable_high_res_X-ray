...
class find(object):
  ...
  def __init__(self,
    ...
    for p in bond_proxies_simple:
      ...
      if(ei in Ds): d_bonded_to.setdefault(i, []).append(atoms[j])
      if(ej in Ds): d_bonded_to.setdefault(j, []).append(atoms[i])
    ...
    for p in pp:
      ...
      if(ei in Hs):
        ...
        A = atoms[j]
        for a in d_bonded_to[D.i_seq]:
          if a.element not in Hs:
            C.append(atoms[a.i_seq])
        atom_i = make_atom_id(atom = H, index = i)
        atom_j = make_atom_id(atom = A, index = j)
        if(j in a_bonded_to.keys()):
  ...
  def as_restraints(self, file_name, 
                    dist_H_A_ideal=None, sigma_dist=0.05,
                    dist_A_D_ideal=2.95,
                    a_DHA_ideal=153.30, sigma_angle=5.0,
                    a_YAH_ideal=147.15, 
  ...
  geometry_restraints.edits{
    ...
    for r in self.result: 
      ...
      print (r.H.id_str(),r.D.id_str(),r.A.id_str())
      H       = r.H
      D       = r.D
      A       = r.A
      Y       = r.Y
      ...
      if H.chain() is None:continue
      H_str = "chain %s and resseq %s and name %s" % (
        H.chain().id,
        H.parent().parent().resid(),
        H.name)
      D_str = "chain %s and resseq %s and name %s" % ( 
        D.chain().id,
        D.parent().parent().resid(),
        D.name)
      A_str = "chain %s and resseq %s and name %s" % ( 
        A.chain().id,
        A.parent().parent().resid(),
        A.name)
      if use_defaul_parameters:
        ...
        a_DHA= a_DHA_ideal
        a_YAH= a_YAH_ideal
      d_HA_str=base_bond%(H_str, A_str, symop, d_HA, sigma_dist)
      d_AD_str=base_bond%(D_str, A_str, symop, d_AD, sigma_dist)
      a_DHA_str=base_angle%(D_str, A_str, H_str, a_DHA, sigma_angle)
      #base_Y="Y"
      base_Y="A"
      for y in Y:
        Y_str = "chain %s and resseq %s and name %s" % ( 
          y.chain().id,
          y.parent().parent().resid(),
          y.name)
        a_YAH=A.angle(y,H,deg=True)
        a_YAD=A.angle(y,D,deg=True)
        a_YAH_str=base_angle%(Y_str, A_str, H_str, a_YAH, sigma_angle)
        a_YAD_str=base_angle%(Y_str, A_str, D_str, a_YAD, sigma_angle)
        base_Y=base_Y+a_YAH_str+a_YAD_str
      base_Y=base_Y[1:]
      base_C="d"
      for c in C:
        #print (dir(D_b))
        C_str = "chain %s and resseq %s and name %s" % ( 
          c.chain().id,
          c.parent().parent().resid(),
          c.name)
        #print (C_str)
        a_CDA=D.angle(c,A,deg=True)
        a_CDH=D.angle(c,H,deg=True)
        a_CDA_str=base_angle%(C_str, D_str, A_str, a_CDA, sigma_angle)
        #a_CDH_str=base_angle%(C_str, D_str, H_str, a_CDH, sigma_angle)
        base_C=base_C+a_CDA_str
        #print (base_C)
      base_C=base_C[1:] 
      base=(d_HA_str+a_DHA_str+base_Y)
      base_str=base_str+base
    base_str=base_str[1:]
    top_str=top%base_str
    print (top_str)
    file_name=file_name[:4] + ".eff"
